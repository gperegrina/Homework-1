...
/*	
	...
	glBegin(GL_TRIANGLE_FAN);
		glVertex2f(game->circle.center.x, game->circle.center.y); // center of circle
		for(i = 0; i <= triangleAmount;i++) { 
			glVertex2f(
		            vert[i].x + (game->circle.radius * cos(i *  twicePi / triangleAmount)), 
			    vert[i].y + (game->circle.radius * sin(i * twicePi / triangleAmount))
			);
		}
	//glEnd();
*/
